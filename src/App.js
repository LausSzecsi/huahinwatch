...
const HuaHinWatch = () => {
  ...
  useEffect(() => {
    ...
    const savedNick = localStorage.getItem('huahin_user_nickname');
  ...
  const handleReportSubmit = () => {
    ...
    const reporterName = userNickname || `Reporter_${Math.floor(Math.random() * 1000)}`;
    const report = {
      id: Date.now(),
      area: newReport.area,
    ...
    if (!userNickname) {
      localStorage.setItem('huahin_user_nickname', reporterName);
      setUserNickname(reporterName);
    }
    setNewReport({ area: '', description: '', time: '' });
  };

  const confirmReport = (reportId) => {
    const updated = outageReports.map(r => 
      r.id === reportId ? { ...r, confirmations: (r.confirmations || 1) + 1 } : r
    );
    setOutageReports(updated);
    localStorage.setItem('huahin_outage_reports', JSON.stringify(updated));
  };
  ...
  outageReports.forEach(r => {
    reporterStats[r.reporter] = (reporterStats[r.reporter] || 0) + 1;
  });
  ...
  return (
    <div style={{ background: '#0f172a', color: '#f1f5f9', minHeight: '100vh', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif' }}>
        ...
        <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
          <div style={{ marginBottom: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              ...
              <p style={{ fontSize: '0.9rem', color: '#cbd5e1', margin: '5px 0 0 0' }}>Community-powered outage reports</p>
            </div>
            {userNickname && <div style={{ fontSize: '0.9rem', color: '#cbd5e1' }}>👤 {userNickname}</div>}
          </div>
      ...
      <main style={{ maxWidth: '1400px', margin: '0 auto', padding: '30px 20px' }}>
        ...
        {currentPage === 'home' && (
          <div>
            ...
            <div style={{ background: 'linear-gradient(135deg, #3b82f6 0%, #10b981 100%)', borderRadius: '16px', padding: '50px 30px', marginBottom: '40px', textAlign: 'center' }}>
              ...
              <p style={{ fontSize: '1.2rem', color: 'rgba(255,255,255,0.9)', margin: '0', lineHeight: '1.6' }}>🚨 Real-time power outages reported by the community</p>
            </div>
            ...
            {/* RECENT REPORTS BANNER */}
            {outageReports.length > 0 && (
              <div style={{ background: 'rgba(239, 68, 68, 0.1)', border: '2px solid #ef4444', borderRadius: '12px', padding: '20px', marginBottom: '40px' }}>
                <h3 style={{ fontSize: '1.1rem', fontWeight: '700', margin: '0 0 15px 0' }}>🚨 Latest Reports</h3>
                <div style={{ display: 'grid', gap: '12px' }}>
                  {outageReports.slice(0, 3).map(report => (
                    <div key={report.id} style={{ background: 'rgba(30, 41, 59, 0.6)', padding: '15px', borderRadius: '8px', borderLeft: '4px solid #ef4444', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <div>
                        <p style={{ fontWeight: '700', margin: '0 0 5px 0' }}>⚠️ {report.area}</p>
                        <p style={{ fontSize: '0.85rem', color: '#94a3b8', margin: '0' }}>Reported by {report.reporter}</p>
                      </div>
                      <button onClick={() => confirmReport(report.id)} style={{ padding: '8px 12px', background: '#3b82f6', border: 'none', color: '#fff', borderRadius: '6px', cursor: 'pointer', fontSize: '0.85rem', fontWeight: '600' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '20px', marginBottom: '40px' }}>
              ...
              <div style={{ background: outageReports.length > 0 ? 'rgba(239, 68, 68, 0.1)' : 'rgba(16, 185, 129, 0.1)', borderRadius: '12px', padding: '25px', border: `2px solid ${outageReports.length > 0 ? '#ef4444' : '#10b981'}` }}>
                <h3 style={{ fontSize: '0.95rem', color: '#cbd5e1', margin: '0 0 15px 0', fontWeight: '600' }}>📡 Community Reports</h3>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '15px' }}>
              ...
              <button onClick={() => setCurrentPage('report_outage')} style={{ padding: '16px', background: 'linear-gradient(135deg, #ef4444, #dc2626)', color: '#fff', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: '700', fontSize: '1rem', transition: 'all 0.3s' }}>Report Outage</button>
        ...
        {currentPage === 'power_status' && (
          <div>
            ...
            {/* LIVE REPORTS */}
            <div style={{ background: 'rgba(30, 41, 59, 0.6)', borderRadius: '12px', padding: '20px', border: '2px solid #ef4444' }}>
              <h3 style={{ fontSize: '1.1rem', fontWeight: '700', margin: '0 0 15px 0' }}>🚨 Live Community Reports ({outageReports.length})</h3>
              {outageReports.length === 0 ? (
                <p style={{ color: '#94a3b8', margin: '0' }}>✓ No outages reported</p>
              ) : (
                <div style={{ display: 'grid', gap: '12px' }}>
                  {outageReports.slice(0, 10).map(report => (
                    <div key={report.id} style={{ padding: '15px', background: 'rgba(239, 68, 68, 0.1)', borderRadius: '8px', borderLeft: '4px solid #ef4444' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: '8px' }}>
                        <div>
                          <p style={{ fontWeight: '600', margin: '0 0 5px 0' }}>⚠️ {report.area}</p>
                          <p style={{ fontSize: '0.85rem', color: '#94a3b8', margin: '0' }}>👤 {report.reporter} {report.time && `@ ${report.time}`}</p>
                        </div>
                        <button onClick={() => confirmReport(report.id)} style={{ padding: '6px 12px', background: '#3b82f6', border: 'none', color: '#fff', borderRadius: '6px', cursor: 'pointer', fontSize: '0.8rem', fontWeight: '600' }}>
                          ✓ {report.confirmations || 1}
                        </button>
                      </div>
                      {report.description && <p style={{ fontSize: '0.85rem', color: '#cbd5e1', margin: '0', fontStyle: 'italic' }}>"{report.description}"</p>}
                    </div>
                  ))}
                </div>
              )}
        ...
        {currentPage === 'report_outage' && (
          <div style={{ maxWidth: '600px', margin: '0 auto' }}>
            ...
            <div style={{ background: 'rgba(30, 41, 59, 0.6)', borderRadius: '12px', padding: '30px', border: '2px solid #3b82f6' }}>
              <p style={{ color: '#cbd5e1', margin: '0 0 25px 0' }}>Help the community! Report outages and see confirmations from other people.</p>
              
              <div style={{ display: 'grid', gap: '20px' }}>
                {!userNickname && (
                  <div>
                    <label style={{ display: 'block', marginBottom: '8px', fontWeight: '600' }}>Your nickname (optional)</label>
                    <input type="text" value={userNickname} onChange={(e) => setUserNickname(e.target.value)} placeholder="e.g., John_HuaHin" style={{ width: '100%', padding: '12px', border: '2px solid #475569', borderRadius: '6px', fontSize: '1rem', fontFamily: 'inherit', background: '#1e293b', color: '#f1f5f9' }} />
                  </div>
                )}
                ...
                <div>
                  ...
                  <textarea value={newReport.description} onChange={(e) => setNewReport({ ...newReport, description: e.target.value })} placeholder="When did it start? Estimated time to fix? How many people affected?" style={{ width: '100%', padding: '12px', border: '2px solid #475569', borderRadius: '6px', fontSize: '1rem', minHeight: '100px', fontFamily: 'inherit', background: '#1e293b', color: '#f1f5f9', resize: 'vertical' }} />
                </div>

                <button onClick={handleReportSubmit} style={{ padding: '14px', background: 'linear-gradient(135deg, #ef4444, #dc2626)', border: 'none', color: '#fff', borderRadius: '6px', cursor: 'pointer', fontWeight: '700', fontSize: '1rem' }}>
                  🚨 Report Outage
                </button>
              </div>

              {outageReports.length > 0 && (
                <div style={{ marginTop: '30px', paddingTop: '30px', borderTop: '1px solid rgba(75, 85, 99, 0.3)' }}>
                  <h3 style={{ fontSize: '1rem', fontWeight: '600', margin: '0 0 15px 0' }}>Your Recent Reports</h3>
                  <div style={{ display: 'grid', gap: '10px' }}>
                    {outageReports.filter(r => r.reporter === userNickname).slice(0, 3).map(report => (
                      <div key={report.id} style={{ padding: '10px', background: 'rgba(59, 130, 246, 0.1)', borderRadius: '6px', fontSize: '0.9rem' }}>
                        <strong>{report.area}</strong> - {report.confirmations || 1} people confirmed ✓
                      </div>
                    ))}
                  </div>
                </div>
        ...
        {currentPage === 'report_outage' && topReporters.length > 0 && (
          <div style={{ marginTop: '40px', maxWidth: '600px', margin: '40px auto 0' }}>
            <h3 style={{ fontSize: '1.3rem', fontWeight: '700', margin: '0 0 20px 0' }}>🏆 Top Reporters This Week</h3>
            <div style={{ display: 'grid', gap: '12px' }}>
              {topReporters.map((reporter, idx) => (
                <div key={idx} style={{ background: 'rgba(30, 41, 59, 0.6)', padding: '15px', borderRadius: '8px', border: '2px solid #3b82f6', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    <span style={{ fontSize: '1.5rem' }}>
                      {idx === 0 ? '🥇' : idx === 1 ? '🥈' : idx === 2 ? '🥉' : '⭐'}
                    </span>
                    <span style={{ fontWeight: '700' }}>{reporter[0]}</span>
                  </div>
                  <span style={{ fontSize: '1.2rem', fontWeight: '700', color: '#60a5fa' }}>{reporter[1]} reports</span>
                </div>
              ))}
            </div>
          </div>
              ...
              {[
                ...
                <div key={idx} style={{ background: 'rgba(30, 41, 59, 0.6)', borderRadius: '12px', overflow: 'hidden', border: '2px solid #3b82f6' }}>
                  ...
                  <div style={{ padding: '15px' }}>
                    ...
                    <p style={{ fontSize: '0.85rem', color: '#94a3b8', margin: '0' }}>Live feed coming soon</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      <footer style={{ background: 'rgba(15, 23, 42, 0.8)', borderTop: '2px solid #3b82f6', padding: '25px 20px', textAlign: 'center', color: '#64748b', fontSize: '0.85rem', marginTop: '60px' }}>
        ...
        <p style={{ margin: '0' }}>🚨 Real-time community-powered power outage reports</p>
      </footer>
    </div>
  );
};
